...
const CustomerButtons = ({ recordType, openNewTab }) => {
  ...
  const handleButton = (type) => {
    ...
    if (type === 'Firewall') { setCurrentComponent('Firewall') }
  ...
  return (
    <>
      ...
      <div className="ms-start d-grid gap-2 d-md-block mb-3">
        ...
        <button className="btn btn-sm btn-tabi-logo mx-1 mb-3" onClick={() => handleButton('Firewall')} disabled={currentComponent === 'Router'}>Add Firewall</button>
      ...
      {currentComponent === 'Firewall' && <FirewallForm recordType={recordType} closeComponent={closeComponent}></FirewallForm>}
