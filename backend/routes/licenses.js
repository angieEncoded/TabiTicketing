...
// fetch all active licenses for a customer
router.get("/:customerId", async (req, res, next) => {
  ...
  try {
    ...
    const customerLicense = await License.findAll({
      where: {
        [Op.and]:
          [
    ...
    if (customerLicense.length < 1) {
      return res.json({ status: "500", message: "There are no contacts to fetch for this customer." })
    }

    return res.json({ status: 200, message: "Successfully fetched", customerLicense: customerLicense });
...
router.post('/:recordType/:id', upload.single('license_file'), validateNewLicense, async (req, res, next) => {
  ...
  try {
    ...
    if (recordType === 'customer') {
      customerLicense = await License.create({ uuid: uuidv4(), customerId: id, ...data })
      return res.json({ status: 200, message: "Successfully saved", customerLicense: customerLicense });
    }
